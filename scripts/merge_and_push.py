...
def push_model(base_model_path: str, destination: str,
    ...
    final_model = AutoModelForCausalLM.from_pretrained(base_model_path, torch_dtype=torch_dtype)
    if peft_path:
...
def main():
    ...
    parser.add_argument('--fp16', default=False, action='store_true', help='Whether to use fp16. If not, uses bf16.')
    args = parser.parse_args()

    if args.token:
        hub.login(args.token, add_to_git_credential=False)

    push_model(args.base, args.destination, args.tokenizer, args.peft, args.fp16)
